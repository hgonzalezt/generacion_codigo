...
public class ProyectoControllerTest {
  ...
  void testGetAllProyectos() throws Exception {
    Proyecto proyecto =
        new Proyecto("Sistema de Ventas", "Descripción", LocalDate.of(2024, 11, 1), true);

    Mockito.when(proyectoService.findAll()).thenReturn(List.of(proyecto));

    mockMvc
        .perform(get("/api/proyectos"))
  ...
  void testGetProyectoById() throws Exception {
    ...
    Proyecto proyecto =
        new Proyecto("E-learning", "Cursos en línea", LocalDate.of(2025, 2, 10), true);

    Mockito.when(proyectoService.findById(id)).thenReturn(proyecto);

    mockMvc
        .perform(get("/api/proyectos/" + id))
        .andExpect(status().isOk())
  ...
  void testDeleteProyecto() throws Exception {
    ...
    mockMvc.perform(delete("/api/proyectos/" + id)).andExpect(status().isOk());
  }
}
